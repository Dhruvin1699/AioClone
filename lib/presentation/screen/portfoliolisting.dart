    ...
    try {
      ...
      if (selectedFilterIds.isEmpty) {
        ...
        if (response.statusCode == 200) {
          ...
          await Future.forEach(gridData, (item) async {
            await _downloadAndSaveImage(item);
            // Additional code for saving data to the database if needed
          });
          ...
          for (var item in gridData) {
            // String? techID = item.techMapping?.isNotEmpty == true ? item.techMapping![0].techID : null;
        ...
        // List<Data> filteredData = await _databaseHelper.fetchDataByDomainAndTechID(selectedFilterIds[0]);
